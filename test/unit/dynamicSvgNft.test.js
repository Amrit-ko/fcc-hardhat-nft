...
!developmentChains.includes(network.name)
    ? describe.scip
    : describe("DynamicSvgNft Unit Test", async function () {
          let dynamicSvg, mockV3Aggregator, accounts, deployer
          ...
          describe("Constructor", function () {
              it("Set correct values", async () => {
                  assert.equal(await dynamicSvg.getLowSVG(), lowSVGImageUri)
                  assert.equal(await dynamicSvg.getHighSVG(), highSVGimageUri)
                  assert.equal(await dynamicSvg.getPriceFeed(), mockV3Aggregator.target)
              })
          })
          describe("mintNft function", function () {
              it("mints Nft according to priceFeed", async () => {
                  const [, price, , ,] = await mockV3Aggregator.latestRoundData()
                  await dynamicSvg.mintNft(price - 108n)
                  assert.equal(await dynamicSvg.tokenURI(0), lowTokenUri)
                  await dynamicSvg.mintNft(price + 108n)
                  assert.equal(await dynamicSvg.tokenURI(1), highTokenUri)
              })
          })
      })
